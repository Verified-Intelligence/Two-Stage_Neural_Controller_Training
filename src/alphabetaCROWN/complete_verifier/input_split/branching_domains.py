...
class UnsortedInputDomainList(InputDomainList):
    ...
    def add(
        ...
        print(f"Verified ratio: {1 - len(torch.nonzero(remaining_index)) / batch}")
        # Also check the ratio verified by the third condition
        verified_by_V_dot = (lb - threshold)[:, 2] > 0
        print(f"Verified by V_dot: {torch.sum(verified_by_V_dot) / batch}")
        # Save the boxes with the index verified by V_dot
        verified_x_L = dm_l[verified_by_V_dot]
        verified_x_U = dm_u[verified_by_V_dot]
        mean_verified_gap = torch.mean(verified_x_U - verified_x_L, dim=0)
        print(f"Mean verified gap: {mean_verified_gap}")
        # append the tensors
