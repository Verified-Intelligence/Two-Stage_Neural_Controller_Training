...
def reordered_batch_verification_input_split(
    ...
    # For lyapunov verification, we update c1 here
    if arguments.Globals["lyapunov"]["c1"] is not None:
        thresholds[:, 1] = -arguments.Globals["lyapunov"]["c1"]
...
def input_bab_parallel(net, init_domain, x, rhs=None,
    ...
    # FIXME: Getting lA directly from the nodes is probably not a good idea.
    # We do not know if the "lA" stored at the node is of the propagation starting
