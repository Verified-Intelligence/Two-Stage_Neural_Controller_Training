...
class ABCROWN:
    ...
    def main(self, interm_bounds=None):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            # key_list = model_ori.state_dict().keys()
            # controller_l1_norm = 0
            # lyapunov_l1_norm = 0
            # for key in key_list:
            #     if key.startswith('controller'):
            #         controller_l1_norm += torch.norm(
            #             model_ori.state_dict()[key], p=1).item()
            #     if key.startswith('lyapunov'):
            #         lyapunov_l1_norm += torch.norm(
            #             model_ori.state_dict()[key], p=1).item()
            # print(f'Model name: {arguments.Config["model"]["path"]}')
            # print(f'Controller L1 norm: {controller_l1_norm}')
            # print(f'Lyapunov L1 norm: {lyapunov_l1_norm}')
