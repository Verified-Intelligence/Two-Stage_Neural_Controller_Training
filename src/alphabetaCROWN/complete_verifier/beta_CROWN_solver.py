...
class LiRPANet:
    def __init__(self, model_ori, in_size, c=None, device=None,
        ...
        self.net = BoundedModule(
            ...
            bound_opts={
                ...
                'mul': {'middle': solver_args['crown']['mul_middle']},
    ...
    def build(self, input_domain, x,
        ...
        if not branching_args['input_split']['enable'] or branching_input_and_activation:
            # Only do this if we are splitting activation
        ...
        if bounding_method == 'alpha-crown':
            ...
            with torch.no_grad():
                if bounding_method == 'init-crown':
                    ...
                    visualize_bound = False
                    if visualize_bound:
                        test_model = LiRPANet(self.model_ori, self.input_shape).net
                        x_L = x_expand.ptb.x_L
                        x_U = x_expand.ptb.x_U
                        x_rand = torch.rand(10000, 4, device=x_L.device) * (x_U - x_L) + x_L
                        test_model(x_rand)
                        for node in test_model.nodes():
                            if hasattr(node, 'forward_value'):
                                self.net[node.name].forward_value = node.forward_value
                        self.net.visualize('cartpole_nomoe_tanh_debug', print_bounds=True)
